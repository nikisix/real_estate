...
#insert int between these
SUMMARY_PATH = ['items', 'summary'] #extract Beds, Baths and Sq. Ft. from here
LINK_PATH = ['items', 'link'] 
TITLE_PATH = ['items', 'title'] #extract price and address

BED_KEYWORD='Bed'
BATH_KEYWORD='Bath'
SQFT_KEYWORD='Sq. Ft.'
 
def extract_num_preceeded_by_word(text, word):
    if(re.search(word+'.*\d+', text)):
        uword = re.search(word+'.*\d+', text).group()
        sword = str(uword)
...
for feed_url in feed_urls:
    ...
    for i in range(0, d[TITLE_PATH[0]].__len__()):
        """
        try:
            d['entries'][i]
        except IndexError:
            break
        """
        link = str(d[LINK_PATH[0]][i][LINK_PATH[1]])
        summary = d[SUMMARY_PATH[0]] [i] [SUMMARY_PATH[1]]
        title = d[TITLE_PATH[0]] [i] [TITLE_PATH[1]]
        #value = u'2815 Kensington Ave, Kansas City, MO 64128, $24,900 3 beds, 2 baths'
        sqft = extract_num_preceeded_by_word(summary, SQFT_KEYWORD)
        beds = extract_num_preceeded_by_word(summary, BED_KEYWORD)
        baths = extract_num_preceeded_by_word(summary, BATH_KEYWORD)
